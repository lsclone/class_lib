...
**
**	tcp发包有机制, 如果data length > 1448, 会自动拆分;
**	如果data length < 1448, 会等待一定时间, 如果有新增data, 会组成1448字节发送, 超时则直接发送当前字节数据。
**	由于MTU值可以修改，故mss不固定为1448字节，所以BUFF_SIZE设置为1024。
**
**	如果在局域网内，由于路由器和主机的MTU皆可配置，故建议统一数值(例如1500)，此时，BUFF_SIZE设置为1448。
*/
